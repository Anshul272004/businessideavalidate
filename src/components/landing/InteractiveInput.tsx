...
const InteractiveInput = () => {
  ...
  // Auto-type demo text when user views section
  useEffect(() => {
    if (!isFocused) return;
    let i = 0;
  ...
  return (
    <section className="luxury-container py-24 relative z-10">
      ...
      >
        <p className="text-sm text-primary mb-4 uppercase tracking-widest font-medium">Try It Now</p>
        ...
        >
          <div className="flex items-center gap-4 px-6 py-5 rounded-xl bg-card/80">
            ...
            <button className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium flex items-center gap-1.5 hover:bg-primary/90 transition-colors haptic-click">
              Analyze
                ...
                <div className="pt-4 border-t border-border/50 space-y-2">
                  {analysisSteps.map((step, i) => (
                    <motion.div
                      ...
                      className={`flex items-center gap-2 text-sm ${i < currentStep ? "text-success" : i === currentStep ? "text-primary" : "text-muted-foreground/30"}`}
                    >
                      {i < currentStep ? (
